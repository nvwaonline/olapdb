...
public class Util {
	...
	public static void deleteGlobalValue(String valueName){
		Delete delete = new Delete(Bytes.toBytes(valueName));
		delete.addColumn(Bytes.toBytes(Obase.FAMILY_ATTR),Bytes.toBytes("value"));
		try {
			Obase.getIncrementTable().delete(delete);
		} catch (IOException e) {
	...
	public static byte[] getGlobalValue(String valueName){
		...
		try {
			Result r = Obase.getIncrementTable().get(inc);
	...
	public static <T> Stream<List<T>> convertToCoarseStream(Stream<T> fineStream, int batchSize) {
		Spliterator<T> spliterator = fineStream.spliterator();
		int characteristics = spliterator.characteristics();
		long estimatedSize = spliterator.estimateSize();

		return StreamSupport.stream(new Spliterators.AbstractSpliterator<List<T>>(estimatedSize, characteristics) {
