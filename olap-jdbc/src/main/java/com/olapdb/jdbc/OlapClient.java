...
import com.olapdb.jdbc.json.PreparedQueryRequest;
import com.olapdb.jdbc.json.SQLResponseStub;
import com.olapdb.jdbc.json.StatementParameter;
import com.olapdb.jdbc.json.TableMetaStub;
...
public class OlapClient implements IRemoteClient {

    private static final Logger logger = LoggerFactory.getLogger(OlapClient.class);

    private final OlapConnectionInfo connInfo;
    private final Properties connProps;
    private HttpClient httpClient;
    private final ObjectMapper jsonMapper;

    public OlapClient(OlapConnectionInfo connInfo) {
    ...
    public static Object wrapObject(String value, int sqlType) {
        ...
        if (sqlType != Types.VARCHAR && value.equals("null")) {
            return null;
        }
    ...
    @Override
    public OlapMeta.KMetaProject retrieveMetaData(String project) throws IOException {
        ...
        try {
            ...
            List<OlapMeta.KMetaTable> tables = convertMetaTables(tableMetaStubs);
            List<OlapMeta.KMetaSchema> schemas = convertMetaSchemas(tables);
            List<OlapMeta.KMetaCatalog> catalogs = convertMetaCatalogs(schemas);
            return new OlapMeta.KMetaProject(project, catalogs);
        } finally {
           get.releaseConnection();
        }
    }

    private List<OlapMeta.KMetaCatalog> convertMetaCatalogs(List<OlapMeta.KMetaSchema> schemas) {
        Map<String, List<OlapMeta.KMetaSchema>> catalogMap = new LinkedHashMap<String, List<OlapMeta.KMetaSchema>>();
        for (OlapMeta.KMetaSchema schema : schemas) {
            List<OlapMeta.KMetaSchema> list = catalogMap.get(schema.tableCatalog);
            if (list == null) {
                list = new ArrayList<OlapMeta.KMetaSchema>();
                catalogMap.put(schema.tableCatalog, list);
            }
            list.add(schema);
        }

        List<OlapMeta.KMetaCatalog> result = new ArrayList<OlapMeta.KMetaCatalog>();
        for (List<OlapMeta.KMetaSchema> catSchemas : catalogMap.values()) {
            String catalog = catSchemas.get(0).tableCatalog;
            result.add(new OlapMeta.KMetaCatalog(catalog, catSchemas));
        }
        return result;
    }

    private List<OlapMeta.KMetaSchema> convertMetaSchemas(List<OlapMeta.KMetaTable> tables) {
        Map<String, List<OlapMeta.KMetaTable>> schemaMap = new LinkedHashMap<String, List<OlapMeta.KMetaTable>>();
        for (OlapMeta.KMetaTable table : tables) {
            String key = table.tableCat + "!!" + table.tableSchem;
            List<OlapMeta.KMetaTable> list = schemaMap.get(key);
            if (list == null) {
                list = new ArrayList<OlapMeta.KMetaTable>();
                schemaMap.put(key, list);
            }
            list.add(table);
        }

        List<OlapMeta.KMetaSchema> result = new ArrayList<OlapMeta.KMetaSchema>();
        for (List<OlapMeta.KMetaTable> schemaTables : schemaMap.values()) {
            String catalog = schemaTables.get(0).tableCat;
            String schema = schemaTables.get(0).tableSchem;
            result.add(new OlapMeta.KMetaSchema(catalog, schema, schemaTables));
        }
        return result;
    }

    private List<OlapMeta.KMetaTable> convertMetaTables(List<TableMetaStub> tableMetaStubs) {
        List<OlapMeta.KMetaTable> result = new ArrayList<OlapMeta.KMetaTable>(tableMetaStubs.size());
    ...
    private OlapMeta.KMetaTable convertMetaTable(TableMetaStub tableStub) {
        List<OlapMeta.KMetaColumn> columns = new ArrayList<OlapMeta.KMetaColumn>(tableStub.getColumns().size());
        for (TableMetaStub.ColumnMetaStub columnStub : tableStub.getColumns()) {
            columns.add(convertMetaColumn(columnStub));
        }
        return new OlapMeta.KMetaTable(tableStub.getTABLE_CAT(), tableStub.getTABLE_SCHEM(), tableStub.getTABLE_NAME(),
                tableStub.getTABLE_TYPE(), columns);
    }

    private OlapMeta.KMetaColumn convertMetaColumn(TableMetaStub.ColumnMetaStub columnStub) {
        return new OlapMeta.KMetaColumn(columnStub.getTABLE_CAT(), columnStub.getTABLE_SCHEM(), columnStub.getTABLE_NAME(),
